        ...
        public static async Task CreateFilesData(IServiceProvider serviceProvider, IConfiguration configuration)
        {
            using (var serviceScope = serviceProvider.GetRequiredService<IServiceScopeFactory>().CreateScope())
            {
                ...
                if (!await context.FileDataTypeGroups.AnyAsync())
                {
                    ...
                    FileDataTypeGroup fileDataTypeGroup18 = new FileDataTypeGroup
                    {
                        FileDataTypeGroupId = (int)FileDataTypeGroupEnum.OVZ,
                        FileDataTypeGroupName = "Информация об обеспечении беспрепятственного доступа в здания образовательной организации"
                    };
                ...
                if (!await context.FileDataTypes.AnyAsync())
                {
                    ...
                    FileDataType fileDataType77 = new FileDataType
                    {
                        FileDataTypeId = (int)FileDataTypeEnum.FondOcenochnihSredstvListPereutverjdeniya,
                        FileDataTypeName = "Документы пользователя - СНИЛС",
                        FileDataTypeGroupId = (int)FileDataTypeGroupEnum.UserDocuments
                    };
                    ...
                    FileDataType fileDataType84 = new FileDataType
                    {
                        FileDataTypeId = (int)FileDataTypeEnum.SvidetelstvoOProfObschAccreditatsii,
                        FileDataTypeName = "Свидетельство о профессионально-общественной аккредитации",
                        FileDataTypeGroupId = (int)FileDataTypeGroupEnum.SvedenEducation
                    };

                    FileDataType fileDataType85 = new FileDataType
                    {
                        FileDataTypeId = (int)FileDataTypeEnum.InfOFormirovaniiPlatiZaProjivanieVObsch,
                        FileDataTypeName = "Информация о формировании платы за проживание в общежитии",
                        FileDataTypeGroupId = (int)FileDataTypeGroupEnum.SvedenDocuments
                    };

                    FileDataType fileDataType86 = new FileDataType
                    {
                        FileDataTypeId = (int)FileDataTypeEnum.InfObObespBesprDostupaVZdaniyaObrOrg,
                        FileDataTypeName = "Информация об обеспечении беспрепятственного доступа в здания образовательной организации",
                        FileDataTypeGroupId = (int)FileDataTypeGroupEnum.OVZ
                    };
                    ...
                    await context.FileDataTypes.AddRangeAsync(
                        ...
                        fileDataType80, fileDataType81, fileDataType82, fileDataType83, fileDataType84, fileDataType85, fileDataType86
                        );
                    await context.SaveChangesAsync();
                }
                #endregion
